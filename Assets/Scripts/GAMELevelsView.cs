...
public class GAMELevelsView : MonoBehaviour
{
    ...
    /// <param name="levelsInfoProvider"></param>
    /// <param name="currentCompletedLevel"></param>
    /// <param name="completedLevelsWithBonus"></param>
    public void Init(in Zenject.SignalBus signalBus, LevelsInfoProvider levelsInfoProvider, int currentCompletedLevel, IList<int> completedLevelsWithBonus){
        ...
        for(int i = 1; i < levelsInfoProvider.LevelsCount + 1; i++){
            ...
            if(completedLevelsWithBonus!=null && completedLevelsWithBonus.Contains(i)) levelButton.BonusReceived();

...
public class AnimatedPanel : MonoBehaviour
{
    ...
    [Header("这个重要！！！不填，则没有滑动;AwakeAction()//setPos也重要")]
    ...
    private void Awake() {
        ...
        Debug.Log("Animated Panel Awake() name==" 
        + gameObject.name + " originPos=" + _awakePosition);
    ...
    private void Start() {
        Debug.Log("Animated Panel Start()");
    ...
    public void CloseInstantly(){
        ...
        Debug.Log($"AnimatePanel() name={gameObject.name} close setPos==" + closedPosition +$" 结果：local{transform.localPosition} world={transform.position}");
    }

    private void AnimatePanel(Vector2 position, float alpha, System.Action onComplete = null){
        Debug.Log($"AnimatePanel() name={gameObject.name} pos={transform.position} to dest={position}");
